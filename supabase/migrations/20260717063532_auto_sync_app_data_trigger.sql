-- Auto-sync: cuando el costosshake guarda en app_data, refleja catálogo +
-- precios + recetas en las tablas relacionales del POS. NO toca stock
-- (inventario_stock) para no pelear con los descuentos por venta.
-- SECURITY DEFINER (inserta bajo RLS). Idempotente. Seguro por MVCC:
-- las ventas en curso ven el estado viejo hasta el commit.

create or replace function fn_sync_app_data() returns void
...
-- Trigger: al guardar en app_data, sincroniza (solo si cambió el jsonb data).
create or replace function trg_sync_app_data() returns trigger
language plpgsql security definer set search_path = public as $$
begin
...
drop trigger if exists app_data_sync on app_data;
create trigger app_data_sync
  after update of data on app_data
  for each row
