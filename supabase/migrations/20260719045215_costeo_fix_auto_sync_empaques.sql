-- Corrige fn_sync_app_data() (trigger automático en cada guardado de app_data)
-- para que también sincronice empaques[] por receta hacia recetas reales,
-- igual que ya hacía supabase/seed/sync-app-data.sql (que es un script manual
-- y NO es lo que corre solo). Aditivo: create or replace, no borra nada.
