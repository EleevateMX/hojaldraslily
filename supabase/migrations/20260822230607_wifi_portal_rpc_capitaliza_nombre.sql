-- Capitaliza el nombre capturado en el portal para que se vea decente en el POS.
...
begin
  ...
  v_nombre := initcap(btrim(regexp_replace(
  ...
  select * into v_cupon from public.cupones
   where cliente_id = v_cliente.id and tipo = 'bienvenida'
     and estado = 'activo' and vence_en > now()
  ...
  return jsonb_build_object(
    'ok', true, 'es_nuevo', v_es_nuevo,
    'cliente', jsonb_build_object('id', v_cliente.id, 'nombre', v_cliente.nombre,
                                  'codigo', v_cliente.codigo, 'mancuernas', v_cliente.mancuernas),
    'cupon', case when v_cupon.id is null then null else jsonb_build_object(
      'codigo', v_cupon.codigo, 'beneficio', v_cupon.beneficio,
      'vence_en', v_cupon.vence_en, 'recien_emitido', v_cupon_new) end);
end;
$$;
